...
bool Grafo::addVertice(int vertice, std::string nome, int color)
{
    /* Adiciona caso já não exista. */
...
void Grafo::setColor(int vertice, int color)
{
    this->vertices.at(vertice).color = color;
}


int Grafo::getColor(int vertice)
{
    return this->vertices.at(vertice).color;
}
        

int * Grafo::getArestas(int vertice, int &sz)
{
    sz = this->vertices.at(vertice).listaAdj.size();
    int * lista = new int[sz];
    for (int i=0; i<sz; i++)
        lista[i] = this->vertices[vertice].listaAdj[i];
